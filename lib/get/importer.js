...
class GetImporter {
    ...
    async import() {
        (0, legacy_logger_1.legacyLog)('import unzip start');
        const tmpDir = path.join(const_1.GET_CACHE_LOC, "data");
        await fs.mkdirp(tmpDir);
        const files = await (0, decompress_1.default)(this.config["rawDir"], tmpDir);
        (0, legacy_logger_1.legacyLog)('import unzip complete', { fileCount: files.length, rawPath: this.config["rawDir"] });
        ...
        (0, legacy_logger_1.legacyLog)('import notes discovered', { notes: notesData.size });
        if (notesData.size === 0) {
            throw new legacy_errors_1.LegacyExportError("PARSE_FAILED", "No note HTML files found in exported ZIP");
        }
        ...
        (0, legacy_logger_1.legacyLog)('import synced memo ids loaded', { count: syncedMemoIds.length });
        // 传递 Map 给 GetCore
        const flomo = new core_1.GetCore(notesData, syncedMemoIds, getTarget);
        const memos = await this.importMemos(flomo);
        (0, legacy_logger_1.legacyLog)('import markdown written', {
        ...
        (0, legacy_logger_1.legacyLog)('import cleanup complete');
        return flomo;
    }
    async importGetFile(filePath, mergeDayFile = true) {
        ...
        const flomo = new core_1.GetCore(getData, syncedMemoIds, getTarget);
